...
| 2026-07-08 | 92cb4b7 | v1_team | 448 | — | — | — | 12.93 | 12.18 | **Carryover baseline (#47).** Close target + `--carryover-k 8` (prior-season rate blend, `max_depth=3`). Pre-registered gate PASS on one-shot 2024/25 holdout: MAE-vs-close 2.92→2.53, gap_rmse +0.81→+0.75, tail>7 4.2%; effect concentrates early-weeks per mechanism (wk≤4 MAE 3.50→2.65). Same-run baseline leg reproduced the #39 row (gap +0.811). Continuity weighting (QB/coach/draft) and disagreement-cap both tested + REJECTED on tune (see #47). Now the `make train` default (`train.py` champion config). |
