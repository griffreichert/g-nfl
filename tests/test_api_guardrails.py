...
from g_nfl.api.main import _candidate_side
...
def test_the_spread_is_flipped_for_the_home_side():
    game = _game(pool=6.5)
    assert _candidate_side(game, "GB")["picked_spread"] == -6.5
    assert _candidate_side(game, "CAR")["picked_spread"] == 6.5
...
def test_the_pool_line_wins_over_the_market_when_both_exist():
    ...
    side = _candidate_side(_game(pool=3.0, market=6.5), "CAR")
    assert side["picked_spread"] == 3.0


def test_the_market_line_stands_in_before_friday():
    side = _candidate_side(_game(pool=None, market=6.5), "CAR")
...
def test_the_gap_is_read_from_the_picked_side():
    ...
    assert _candidate_side(game, "GB")["gap_side"] == "better"
    assert _candidate_side(game, "CAR")["gap_side"] == "worse"


def test_no_market_line_means_no_gap_rather_than_a_zero():
    side = _candidate_side(_game(pool=3.0), "CAR")
...
def test_a_candidate_side_satisfies_every_rule_predicate():
    ...
    side = _candidate_side(_game(pool=9.5, market=9.0), "CAR")
