...
2. Set the five secret env vars when prompted. The blueprint declares all of
   them, so Render asks for each one during setup:
   ...
   - `CORS_ORIGINS` — comma-separated, e.g. `https://<your-app>.vercel.app`.
     The Vercel project does not exist yet on a first deploy; put
     `http://localhost:5173` in and come back to it once Vercel has a domain.
...
3. Deploys automatically on push to `main`, which `render.yaml` pins
