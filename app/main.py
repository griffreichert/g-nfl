"""Fantasy draft board with live league settings (issues #90, #91).

The whole Streamlit app. The picks pages it used to sit behind (make picks,
view picks, manage spreads) moved to the React app in ``web/`` and were deleted
in #91, so this is a single-page app now rather than a multipage one.
...
def run_app():
    """Entry point for the ``app`` console script (see pyproject)."""
